        ...
        if len(keys_arr) > 1:
            ...
            for cell_keys in keys_arr:
                if len(lst3) != len(cell_keys):
                    exit(
                        "文件比中文key少,有缺少的key,请查看json文件长度后使用json_key_compare.py对比"
                    )
            ...
            if isSame:
                ...
                for key in json_keys.keys():
                    ...
                    if len(parts) == 1:
                        ...
                        transformed_parts = [parts[0]] + [
                            str(part[0]).upper() + part[1 : len(part)]
                            for part in parts[1:]
                        ]
                    lowercase_key = "".join(transformed_parts)
                    localeKeyStr = (
                        localeKeyStr
                        + "  static const "
                        + lowercase_key
                ...
                Tool.replace_text(
                    generated_file_path, translationFirst, translationsStr
                )
                # 4.更新supportLocals
