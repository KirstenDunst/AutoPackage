...
class ExcelTool:
    ...
    # 将多级字典转换成以及字典
    @staticmethod 
    def transToDirectDict(tempDict:dict,keyFront:str):
        endDict = {}
        for k,v in tempDict.items():
            if str(v.__class__).__contains__('dict'):
                tempDict = ExcelTool.transToDirectDict(v,keyFront+k)
                endDict.update(tempDict)
            else:
                endDict[keyFront+k] = v
        return endDict
...
if __name__ == '__main__':
    ...
    locales_names = input('将需要转化对应的列与对应的json命名(多个用,隔开)下标从0开始:(eg: 1:en_US,0:zh_CN):')
    #1:en_US,2:de_DE,3:es_ES,4:fr_FR,5:pt_PT,6:it_IT,7:pl_PL
    ...
    ws = workbook[sheet_names[sheet_index]]
    ...
    if hasData : 
        # title_map = {'en_US':'英文','de_DE':'德语','es_ES':'西班牙语','fr_FR':'法语','pt_PT':'葡萄牙语','it_IT':'意大利语','pl_PL':'波兰语'}
        outwb = Workbook()
        ...
        for k,v in file_map.items():
            ...
            compare_file_map[k] = ExcelTool.transToDirectDict(temp_json_content,'')
        outws.append(titles)
        
        direct_chinese_dict = ExcelTool.transToDirectDict(no_trans_chinese,'')
    ...
    for k,value in file_map.items():
        file_path = source_dir+k+'.json'
        if os.path.exists(file_path):
            skip = input('检测到文件已存在:'+file_path+'跳过「S」?还是覆盖「O」?,还是合并「M」,或其他字符退出执行:')
